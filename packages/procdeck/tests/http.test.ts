...
import type { HttpDigestGroup, HttpResult } from "../src/http-log.ts"
...
}).listen(process.env.PORT, () => console.log("serving on " + process.env.PORT));
`.trim()
...
describe("http observer", () => {
  test("the assigned port is the observer: traffic through it is served and captured", async () => {
    ...
    const hello = result.exchanges.find((e) => e.path === "/hello")
  ...
  test("bodies on request: text captured and truncatable, auth headers redacted", async () => {
    ...
    const echo = result.exchanges.at(-1)!
  ...
  test("status filter narrows to failures", async () => {
    ...
    expect(notFound.exchanges.every((e) => e.status === 404)).toBe(true)
