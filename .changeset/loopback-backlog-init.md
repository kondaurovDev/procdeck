...
itself. **`procdeck init`:** writes a first `procdeck.config.json` from your workspace —
one proc per package with a `dev` script, run through the package manager your lockfile
points at.
